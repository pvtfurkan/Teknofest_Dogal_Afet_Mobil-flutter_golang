...
import 'package:dogalafetmobil/animations/bottomAnimation.dart';
...
class _PhoneBookState extends State<PhoneBook> {
  ...
  saveContacts() async {
    GetStorage box = GetStorage();
    List<dynamic> numbers = box.read("numbers");
    if (_userSelectedContacts.isNotEmpty) {
      ...
      await box.write("numbers", numbers);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        ...
        child: Text("Kaydet",style: TextStyle(fontSize:10),),
      ),
  ...
  String refactorPhoneNumbers(String phone) {
    ...
    if (newPhone.length == 11) {
      newPhone = "+90" + newPhone.substring(1, newPhone.length);
    }
    if (newPhone.length > 12) {
